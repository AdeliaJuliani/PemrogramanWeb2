...
<div class="px-8 mb-12" id="rekomendasiProduk">
  <span id="cartCountProdukBuah"></span>
  <h2 class="text-center text-5xl font-bold text-blue-600 mb-6">Buah - Buahan</h2>
  <div id="product-list" class="space-y-6"></div>
</div>

<script>
    $(document).ready(function(){
        // Panggil produk berdasarkan kode_varchar 'BA'
        fetchProducts('BA');
    });
...
$(document).on('click', '.btn-tambah', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (data.success) {
                $('#cartCountProdukBuah').text(data.total_items);
